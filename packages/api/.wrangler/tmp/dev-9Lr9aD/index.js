...
__export(schema_exports, {
  ...
  chainBusinesses: () => chainBusinesses,
  syncLogs: () => syncLogs,
  userSessions: () => userSessions
});
var businesses = sqliteTable("businesses", {
...
var chainBusinesses = sqliteTable("chain_businesses", {
  id: text("id").primaryKey(),
  name: text("name").notNull(),
  patterns: text("patterns"),
  // JSON array of name patterns to match
  category: text("category"),
  parentCompany: text("parent_company"),
  confidenceScore: integer("confidence_score").default(100),
  // 0-100 matching confidence
  createdAt: text("created_at").default(sql`CURRENT_TIMESTAMP`)
});
var analyticsEvents = sqliteTable("analytics_events", {
  id: text("id").primaryKey(),
  extensionId: text("extension_id"),
  // anonymous extension identifier
  eventType: text("event_type").notNull(),
  // view, click, filter_toggle, etc.
  businessId: text("business_id"),
  metadata: text("metadata"),
  // JSON additional data
  timestamp: text("timestamp").default(sql`CURRENT_TIMESTAMP`)
});
var userSessions = sqliteTable("user_sessions", {
  id: text("id").primaryKey(),
...
var syncLogs = sqliteTable("sync_logs", {
  id: text("id").primaryKey(),
  syncType: text("sync_type").notNull(),
  // businesses, chains, full
...
function createDatabase(env = {}) {
  ...
  const drizzleDb = drizzle(client, { schema: schema_exports });
...
router.get("/nearby", async (c) => {
  try {
    ...
    })).filter((business) => business.distance <= radius).sort((a, b) => a.distance - b.distance);
    return c.json({
...
router.get("/semantic-search", async (c) => {
  try {
    ...
    const scoredBusinesses = nearbyBusinesses.map((business) => {
    ...
    }).filter((business) => business.relevanceScore > 10).sort((a, b) => b.combinedScore - a.combinedScore).slice(0, limit);
...
router2.get("/", async (c) => {
  try {
    ...
    }).from(chainBusinesses).where(gte(chainBusinesses.confidenceScore, 80));
...
async function updateUserSession(extensionId, events) {
  try {
    const existingSessions = await db.select().from(userSessions).where(eq(userSessions.extensionId, extensionId)).limit(1);
    const sessionData = {
    ...
    if (existingSessions.length > 0) {
      ...
      await db.update(userSessions).set({
        ...
        sessionEnd: (/* @__PURE__ */ new Date()).toISOString()
...
__name(updateUserSession, "updateUserSession");
router3.get("/summary", async (c) => {
  try {
...
app.route("/api/businesses", businesses_default);
app.route("/api/chains", chains_default);
...
// .wrangler/tmp/bundle-6yXpeo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6yXpeo/middleware-loader.entry.ts
