...
// Progress tracking
const PROGRESS_FILE = '../test-data/enrichment-progress-500.json';
...
async function scrapeWithFirecrawl(url) {
  try {
    ...
    // TODO: Replace with actual Firecrawl MCP call when available
    // For now using fetch as fallback but tracking as Firecrawl credit usage
...
async function enrichBusiness(business, db, progress) {
  ...
  try {
    ...
    const scrapeResult = await scrapeWithFirecrawl(business.website);
    progress.firecrawlCreditsUsed++;
...
async function main() {
  console.log('🚀 Firecrawl Enrichment - Processing 500 Businesses');
  console.log('=' .repeat(60));
  
  const progress = loadProgress();
  console.log(`📊 Current Progress: ${progress.processed}/500 processed, ${progress.successful} successful, ${progress.failed} failed`);
  ...
  try {
    ...
    // Get 500 businesses that need enrichment, prioritizing LFA members
    ...
    console.log(`   🔥 Firecrawl credits to use: 500`);
    console.log(`   💰 Estimated total cost: $40 (Firecrawl + OpenAI)`);
    ...
    for (let i = 0; i < businessesToEnrich.length; i++) {
      ...
      if (progress.lastProcessedId === business.id) {
      ...
      if (progress.processed % 10 === 0) {
        ...
        console.log(`\n💾 Progress saved: ${progress.processed}/500 (${progress.successful} successful, ${progress.failed} failed)`);
        console.log(`🔥 Firecrawl credits used: ${progress.firecrawlCreditsUsed}`);
    ...
    console.log(`   🔥 Total Firecrawl credits used: ${progress.firecrawlCreditsUsed}`);
    console.log(`   💰 Total estimated cost: $${progress.estimatedCost.toFixed(2)}`);
    console.log(`   ⏱️  Duration: ${new Date(progress.endTime).getTime() - new Date(progress.startTime).getTime()} ms`);
    
    if (progress.failedBusinesses.length > 0) {
      console.log(`\n⚠️  Failed businesses (${progress.failedBusinesses.length}):`);
      progress.failedBusinesses.slice(0, 5).forEach(fb => {
        console.log(`   - ${fb.name} (${fb.website}): ${fb.error}`);
      });
      if (progress.failedBusinesses.length > 5) {
        console.log(`   ... and ${progress.failedBusinesses.length - 5} more`);
      }
    }
