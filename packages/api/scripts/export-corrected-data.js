  ...
  try {
    ...
    for (let i = 0; i < allBusinesses.length; i += BATCH_SIZE) {
      ...
      for (const business of batch) {
        ...
        const escapedDescription = business.description ? business.description.replace(/'/g, "''") : '';
        const escapedCategory = business.category ? business.category.replace(/'/g, "''") : '';
        const escapedSubcategory = business.subcategory ? business.subcategory.replace(/'/g, "''") : '';
        ...
        const insertSql = `INSERT INTO businesses (
          ...
          category, subcategory, website, phone, local_first_member, 
    ...
    // Write to file
    const outputPath = path.join(process.cwd(), 'corrected-businesses-export.sql');
    ...
    const samplePath = path.join(process.cwd(), 'sample-businesses-export.sql');
    fs.writeFileSync(samplePath, sampleSql.join('\n'));
    
    console.log(`🧪 Sample file created: ${samplePath} (${sampleSize} businesses)`);
    console.log(`💡 Test with sample first, then use full export`);
